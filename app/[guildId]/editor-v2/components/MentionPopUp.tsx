...
const SuggestionIcon = ({ item }: { item: Suggestion }) => {
  ...
  if (item.type === "role") {
    return (
      <div style={{ color: item.color }}>
        <DiscordIcon name="roles" className="w-5 h-5" />
      </div>
    );
  }
  if (item.type === "emoji") {
    return (
      <Image
        src={`https://cdn.discordapp.com/emojis/${item.id}.${item.animated ? "gif" : "png"}`}
        alt=""
  ...
  const channelIconMap: Record<
    number,
    "text" | "voice" | "category" | "announcements" | "stageVoice"
  > = {
    0: "text",
    2: "voice",
    4: "category",
  ...
  return (
    <div className="text-gray-400">
      <DiscordIcon
        name={channelIconMap[item.subType || 0] || "text"}
        className="w-5 h-5"
      />
    </div>
  );
...
export default function MentionInput({
  ...
  const updateCoords = () => {
    ...
    const placement =
      window.innerHeight - (rect.top + relativeTop) < 350 ? "top" : "bottom";
    setCoords({
  ...
  const suggestions = useMemo<Suggestion[]>(() => {
    ...
    const filters: Record<string, () => Suggestion[]> = {
      "@": () => [
        ...serverData.members
          .filter((m) => normalizeText(m.nickname || m.username).includes(q))
          .map((m) => ({
            id: m.id,
        ...
        ...serverData.roles
          .filter((r) => normalizeText(r.name).includes(q))
          .map((r) => ({
            id: r.id,
            name: r.name,
            type: "role" as const,
            color: r.color || "#96a6a8",
          })),
      ],
      "#": () =>
        serverData.channels
          .filter((c) => normalizeText(c.name).includes(q) && c.type !== 4)
          .map((c) => ({
            id: c.id,
            name: c.name,
      ...
      ";": () =>
        serverData.emojis
          .filter((e) => e.name && normalizeText(e.name).includes(q))
          .map((e) => ({
            id: e.id,
            name: e.name ?? "",
            type: "emoji" as const,
      ...
      "\\": () => {
        ...
        const styles = [
          ...
          {
            id: "d",
            name: format({ day: "2-digit", month: "2-digit", year: "numeric" }),
          },
          {
            id: "D",
            name: format({ day: "numeric", month: "long", year: "numeric" }),
          },
          {
            id: "f",
            name: format({
  ...
  const handleInput = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (match) {
      ...
      setState((s) => ({
        ...
        show: true,
        baseTimestamp:
          trigger === "\\" ? Math.floor(Date.now() / 1000) : s.baseTimestamp,
      }));
      updateCoords();
    } else {
    ...
    if (state.show && suggestions.length) {
      if (["ArrowUp", "ArrowDown"].includes(e.key)) {
        ...
        setState((s) => ({
          ...s,
          index:
            (s.index + (e.key === "ArrowDown" ? 1 : -1) + suggestions.length) %
            suggestions.length,
        }));
        return;
      }
  ...
  return (
    <div className="relative w-full">
            ...
            style={{
              ...
              bottom:
                coords.placement === "top"
                  ? (coords.inputHeight || 0) - coords.top
                  : "auto",
              ...
              {suggestions.map((item, idx) => (
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      color: item.type === "role" ? item.color : "inherit",
                    }}
                  >
                    {item.type === "role" && "@"}
                    {item.name}
      ...
      {maxLength && (
        <div
          className={`absolute bottom-1 right-2 text-[10px] pointer-events-none ${isOverLimit ? "text-red-400" : "text-zinc-600"}`}
        >
...
function formatMention(item: Suggestion) {
  switch (item.type) {
    case "user":
      return `<@${item.id}> `;
    case "role":
