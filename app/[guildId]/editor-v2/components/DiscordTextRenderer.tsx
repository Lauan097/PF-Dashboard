...
function formatTimestamp(ts: number, fmt: string): string {
  ...
  if (fmt === "R") {
    ...
    const future = diff > 0;
    if (abs < 60)
      return `${future ? "em" : "há"} ${Math.round(abs)} segundo${Math.round(abs) !== 1 ? "s" : ""}`;
    if (abs < 3600)
      return `${future ? "em" : "há"} ${Math.floor(abs / 60)} minuto${Math.floor(abs / 60) !== 1 ? "s" : ""}`;
    if (abs < 86400)
      return `${future ? "em" : "há"} ${Math.floor(abs / 3600)} hora${Math.floor(abs / 3600) !== 1 ? "s" : ""}`;
  ...
  const patterns: Pattern[] = [
    [
      ...
      (s) => (
        <span
          key={kg.next()}
          className="bg-zinc-700 text-zinc-700 hover:text-zinc-200 hover:bg-zinc-600 rounded-sm px-0.5 cursor-pointer select-none transition-colors duration-150"
        >
          {renderMarkdown(s, kg)}
        </span>
      ),
    ...
    [
      /\*\*\*(.+?)\*\*\*/,
      (s) => (
        <strong key={kg.next()} className="font-bold">
          <em>{renderMarkdown(s, kg)}</em>
        </strong>
      ),
    ],
    [
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    if (m[1] !== undefined) {
      ...
      result.push(
        <code
          key={kg.next()}
          className="bg-[#111214] text-[#d4d4d4] px-1 py-0.5 rounded text-[0.875em] font-mono"
        >
          {m[1]}
        </code>,
      );
      ...
      result.push(
        <span
          key={kg.next()}
          className="bg-[#5865F2]/20 text-[#c9cdfb] rounded px-0.5 hover:bg-[#5865F2]/40 cursor-pointer transition-colors duration-100"
        >
      ...
      const color =
        role?.color && role.color !== "#000000" ? role.color : "#9ca3af";
      result.push(
        <span
          key={kg.next()}
          style={{ color, backgroundColor: color + "30" }}
          className="rounded px-0.5 cursor-pointer transition-colors duration-100"
        >
          @{role?.name || "Cargo"}
        </span>,
      );
      ...
      result.push(
        <span
          key={kg.next()}
          className="bg-zinc-700/60 text-zinc-300 rounded px-1 text-[0.9em] whitespace-nowrap"
        >
  ...
  while (i < lines.length) {
    ...
    if (isListLine(line)) {
      ...
      result.push(
        <div
          key={kg.next()}
          className="border-l-4 border-zinc-500 pl-3 my-0.5 text-zinc-300"
        >
      ...
      result.push(
        <div
          key={kg.next()}
          className="text-base font-bold text-[#f2f3f5] mt-1"
        >
      ...
      result.push(
        <div key={kg.next()}>{renderInline(line, serverData, kg)}</div>,
      );
      i++;
    }
  }
...
export function DiscordTextRenderer({
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    nodes.push(
      <pre
        key={kg.next()}
        className="bg-[#1e1f22] rounded-md p-3 my-1 text-sm font-mono text-[#d4d4d4] overflow-x-auto whitespace-pre-wrap border border-zinc-700/30"
      >
        <code>{m[1]}</code>
      </pre>,
    );
