...
export default function MemberTable({ members, loading = true }: MemberTableProps) {
      ...
      <div className="w-full overflow-x-auto md:overflow-y-hidden flex-1">
        <table className="w-full text-sm text-left text-gray-400">
          <thead className="text-xs uppercase bg-[#18181B] text-gray-500 rounded-lg">
            <tr>
              <th scope="col" className="px-6 py-4 rounded-tl-lg font-semibold tracking-wider">
                <span className="flex items-center gap-1.5"><User2 className="w-4 h-3" /> Membro</span>
              </th>
              <th scope="col" className="px-6 py-4 font-semibold tracking-wider">
                <span className="flex items-center gap-1.5"><Calendar1 className="w-4 h-3" /> Data de Entrada</span>
              </th>
              <th scope="col" className="px-6 py-4 font-semibold tracking-wider">
                <span className="flex items-center gap-1.5"><Activity className="w-4 h-3" /> Atividade</span>
              </th>
          ...
          <AnimatePresence mode="wait" initial={false}>
            <motion.tbody
              key={loading ? 'skeleton' : `page-${currentPage}-count-${currentMembers.length}`}
              initial="initial"
              animate="animate"
              exit="exit"
              variants={variants}
              ...
              {loading ? (
                ...
                >
                    ...
                    <div className="flex items-center gap-4">
                      <div className="relative shrink-0">
                        <Image
                          ...
                          unoptimized
                        />
                        <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-[#3d3d3d] ${getStatusColor(member.status)}`} />
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4 text-gray-400 whitespace-nowrap truncate max-w-60">
                    {member.activity ? (<span className="text-green-400">{member.activity}</span>) : 'Nenhuma Atividade'}
                  </td>
