...
import { Kbd, Dropdown, InputGroup, Separator } from "@heroui/react";
import { CircleCheck, CircleMinus, CloudOff, Filter, Moon, RefreshCw, Search, Trash2, UsersRound } from "lucide-react"; 
...
export default function MembersPage() {
  ...
  const fetchMembers = useCallback(async (isManualRefresh = false) => {
    if (!isManualRefresh) {
      setLoading(true);
    }
    ...
    setAnimLoading(true);
    try {
      const response = await fetch(`/api/data/guild?guildId=${guildId}&type=members`);
      ...
      <div className="min-h-[80vh] p-2 flex flex-col shadow-xl">
        <div className="bg-[#3d3d3d] p-2 rounded-md flex items-center border border-white/5 mb-4">
          ...
          <div className="max-w-80 w-full">
            <InputGroup className="border-[#333] transition-all focus-within:border-blue-500/50">
              <InputGroup.Prefix><Search size={18} className="text-gray-400" /></InputGroup.Prefix>
              <InputGroup.Input
                ref={searchInputRef}
              ...
              />
              <InputGroup.Suffix><Kbd>Alt + P</Kbd></InputGroup.Suffix>
            </InputGroup>
          </div>
          ...
          <div className="ml-auto space-x-2 flex items-center">
            <Dropdown>
              ...
              <Dropdown.Popover className="shadow-2xl min-w-38 border border-white/10" placement="left top">
                <Dropdown.Menu>
                  ...
                  <Dropdown.SubmenuTrigger>
                    ...
                    <Dropdown.Popover className="min-w-38">
                      <Dropdown.Menu>
                        <Dropdown.Item onClick={() => setDateSort("desc")} className={dateSort === "desc" ? "bg-white/10" : ""}>
                          Mais Recentes
                        </Dropdown.Item>
                        <Dropdown.Item onClick={() => setDateSort("asc")} className={dateSort === "asc" ? "bg-white/10" : ""}>
                          Mais Antigos
                        </Dropdown.Item>
                      </Dropdown.Menu>
                    </Dropdown.Popover>
                  ...
                  <Dropdown.SubmenuTrigger>
                    ...
                    <Dropdown.Popover className="min-w-38">
                      <Dropdown.Menu>
                        <Dropdown.Item onClick={() => setStatusFilter("online")} className={statusFilter === "online" ? "bg-white/10" : ""}>
                          Disponível
                          <CircleCheck size={16} className="text-emerald-500 ml-auto" />
                        </Dropdown.Item>
                        <Dropdown.Item onClick={() => setStatusFilter("idle")} className={statusFilter === "idle" ? "bg-white/10" : ""}>
                          Ausente
                          <Moon size={16} className="text-yellow-500 ml-auto" />
                        </Dropdown.Item>
                        <Dropdown.Item onClick={() => setStatusFilter("dnd")} className={statusFilter === "dnd" ? "bg-white/10" : ""}>
                          Não Perturbe
                          <CircleMinus size={16} className="text-red-500 ml-auto" />
                        </Dropdown.Item>
                        <Dropdown.Item onClick={() => setStatusFilter("offline")} className={statusFilter === "offline" ? "bg-white/10" : ""}>
                          Offline
                          <CloudOff size={16} className="text-gray-500 ml-auto" />
                        </Dropdown.Item>
                      </Dropdown.Menu>
                    </Dropdown.Popover>
                  </Dropdown.SubmenuTrigger>
            ...
            <TooltipButton 
              ...
              isIconOnly 
              icon={<RefreshCw className={Animloading ? "animate-spin text-blue-400" : ""} />} 
