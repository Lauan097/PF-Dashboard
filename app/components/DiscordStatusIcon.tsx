const icons: Record<string, (size: number, className?: string) => React.ReactNode> = {
  online: (size, className) => (
    <svg width={size} height={size} viewBox="0 0 680 680" xmlns="http://www.w3.org/2000/svg" className={className}>
...
export default function DiscordStatusIcon({ name, size = 18, className }: DiscordStatusIconProps) {
  const render = icons[name];

  if (!render) return null;

  return <>{render(Number(size), className)}</>;
}